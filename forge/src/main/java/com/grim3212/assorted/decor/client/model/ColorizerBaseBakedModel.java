...
public abstract class ColorizerBaseBakedModel extends BakedModelWrapper<BakedModel> {
    ...
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, @Nonnull RandomSource rand, @Nonnull ModelData extraData, @Nullable RenderType renderType) {
        ...
        if (extraData.get(ForgeColorizerBlockEntity.BLOCK_STATE) != null) {
            blockState = extraData.get(ForgeColorizerBlockEntity.BLOCK_STATE);
        }
    ...
    public TextureAtlasSprite getParticleIcon(ModelData data) {
        BlockState state = data.get(ForgeColorizerBlockEntity.BLOCK_STATE);
        if (state == null) {
            return this.baseSprite;
        } else if (state == Blocks.AIR.defaultBlockState()) {
            return Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(new ResourceLocation(Constants.MOD_ID, "block/colorizer"));
        }
